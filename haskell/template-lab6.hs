...
instance Num a => Monoid (Sum a) where
  mempty = Sum 0 
  mappend (Sum sumA) (Sum sumB) = (Sum (sumA + sumB)) 
  
instance Num a => Monoid (Product a) where
  mempty = Product 1 
  mappend (Product prodA) (Product prodB) = (Product (prodA * prodB)) 
...
class Functor f => Foldable f where
  ...
  foldMap xs = (fold . fmap xs) 
  
instance Foldable Rose where
  fold (a :> []) = a
  fold (a :> xs) = mappend a (foldr (mappend) (mempty) (fmap fold xs))
